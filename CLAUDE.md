...
- **UI components:** two separate layers, deliberately not shared.
  ...
  - *Admin panel:* **our own components in `components/admin/ui/`** — no shadcn. The marketing primitives are tuned for a landing page (`components/ui/button.tsx` is `px-8 py-6 text-base` and full-width on mobile), which makes a dense dashboard look oversized.
...
features/
  ...
  projects/
    ...
    constants/Projects.ts     # the public site's mock; the admin one is gone
...
components/
  ...
  admin/                      # admin chrome: AdminShell, AdminSidebar, AdminHeader,
                              ...
                              # SignOutButton
    ui/                       # our own admin component library (PascalCase):
                              # Button, IconButton, Panel, DataTable, TableToolbar,
                              # Pagination, TextInput, SelectInput, Modal, Badge,
                              # Tooltip, Avatar, EmptyState
...
types/
  ...
  AdminUi.ts                   # every props type in components/admin/ui/
public/
scripts/
```
...
- **Role is never determined at the edge.** `proxy.ts` composes `clerkMiddleware()` around the existing next-intl middleware and, for any `/:locale/admin(.*)` path, calls `auth.protect()` — that's a "signed in at all?" check only, resolved locally from the session JWT, no network call. It cannot know `admin` vs `user`, because — same as the backend — role lives only in the backend's own MongoDB, never in a Clerk session claim.
...
- **`features/auth/components/GoogleButton.tsx`** — our own button (inline multi-color "G" SVG, `components/admin/ui/Button` styling), not Clerk's `<SignInWithMetamaskButton>`-style prebuilt. The actual `authenticateWithRedirect()` call stays in each parent view since `signIn` and `signUp` call it on different resources.
...
| Projects | **Admin side wired to the API.** `ProjectsTable` (debounced server search, status/category filters, server paging, delete), `ProjectForm` (loads by id, create/update, gallery-image delete), `ProjectStats` and `RecentProjectsPanel` all run on the hooks; `constants/AdminProjects.ts` is deleted. The **public** `ProjectsSection` is still on the old mock shape — see Pending Decisions. |
...
| Admin panel (chrome) | **Done, and now protected.** `app/[locale]/admin/` with its own shell: a sidebar that collapses to an icon rail (persisted in `localStorage`), a route-derived breadcrumb, and its own component library in `components/admin/ui/`. `admin/layout.tsx` gates every route on `admin`/`super_admin` — see "Auth & Roles (Clerk)". |
...
- **The public site's English pages scroll horizontally by 27px on a 390px viewport** (`/en`; the Arabic `/ar` measures clean). Same class of bug as the admin panel's, deferred by choice while the admin work was in scope — most likely `components/shared/NavigationOverlay.tsx`'s drawer, which uses the same off-canvas pattern that was just fixed in `AdminMobileSidebar`.
...
- **`components/admin/ui/Field.tsx` only wires `aria-invalid`/`aria-describedby` when its children are passed as a render prop** — and every caller in the panel passes plain JSX instead, so that wiring is silently skipped on every admin form. The error text renders and is visible, but a screen reader is never told the control is invalid, which is the exact gap the component was introduced to close. Fixing it means either converting every call site or having `Field` clone a single child element to inject the props.
...
- Whether the public `features/projects` mock (`Project`, with `year`/`tags`/`color`) is migrated onto the real API shape, and what replaces the three fields the backend has no equivalent for.
